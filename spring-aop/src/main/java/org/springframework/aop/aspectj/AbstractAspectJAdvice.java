...
public abstract class AbstractAspectJAdvice implements Advice, AspectJPrecedenceInformation, Serializable {
	...
	public final synchronized void calculateArgumentBindings() {
		...
		//首次进来的时候，argumentsIntrospected=false，这是需要看parameterTypes的个数
		//第二次进来后，argumentsIntrospected=true，这里直接返回了
		...
		//这时参数数量至少都是大于等于1的，取第一个参数进行处理
		if (maybeBindJoinPoint(parameterTypes[0])//todo 第一个参数是JointPoint类型
				|| maybeBindProceedingJoinPoint(parameterTypes[0])//todo 第一个参数是ProceedingJoinPoint类型，如果参数类型是ProceedingJoinPoint，只有@Around注解的方法才能有这种参数
				|| maybeBindJoinPointStaticPart(parameterTypes[0])) {//todo 第一个参数是JoinPoint.StaticPart类型
			//以上之一，则参数数量减一
			numUnboundArgs--;
		}

		//只有一个参数，并且参数类型是JointPoint、ProceedingJoinPoint、JoinPoint.StaticPart，就不会进入这个分支
		if (numUnboundArgs > 0) {
